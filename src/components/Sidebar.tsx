      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-0.5">
        {project.viewports.map((vp) => (
          <button
            ...
            className={`group w-full flex items-center gap-2 px-2.5 py-2 rounded-lg transition-all text-left ${
          ...
          >
            ...
            <input
              ...
              spellCheck={false}
            />
            <span className="badge badge-accent text-2xs">{vp.items.length}</span>
            {project.viewports.length > 1 && (
              <button
                onClick={(e) => { e.stopPropagation(); if (confirm(`Delete "${vp.name}"?`)) removeViewport(vp.id) }}
                className="opacity-0 group-hover:opacity-100 text-text-muted hover:text-danger transition-fast p-1 rounded"
              >
                <Trash2 size={12} />
              </button>
            )}
